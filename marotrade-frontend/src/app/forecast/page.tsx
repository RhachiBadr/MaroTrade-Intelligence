...
export default function ForecastPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-10 py-6">
        ...
        {MOCK_RESULTS.map((r, i) => (
          <button key={r.country.code} onClick={() => setSelectedIdx(i)}
            className={cn(
              ...
              i === selectedIdx
                ? "bg-text-primary text-white border-text-primary shadow-lg shadow-gray-200"
      ...
      {result.forecast && (
        <div className="grid sm:grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
          {[
            ...
            <div key={label} className="bg-white rounded-3xl border border-border p-6 shadow-sm flex flex-col items-center text-center">
              <div className="p-2 bg-secondary rounded-xl mb-3 text-text-muted">{icon}</div>
      ...
      {/* Chart */}
      <div className="bg-white rounded-[2.5rem] border border-border p-10 shadow-sm animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      {/* Ranking comparison */}
      <div className="bg-white rounded-[2.5rem] border border-border p-10 shadow-sm animate-in fade-in slide-in-from-bottom-6 duration-1000">
